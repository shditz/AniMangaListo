...
const MangaReviews = ({ mangaId }) => {
  const [page, setPage] = useState(1);
  const [preloadedPages, setPreloadedPages] = useState(new Set([1]));

  const { data, error, isLoading, isValidating, mutate } = useSWR(
    `https://api.jikan.moe/v4/manga/${mangaId}/reviews?page=${page}`,
    fetcher,
    {
  ...
  const preloadNextPage = () => {
    ...
    if (!preloadedPages.has(nextPage)) {
      const url = `https://api.jikan.moe/v4/manga/${mangaId}/reviews?page=${nextPage}`;
  ...
  if (error)
    return <div className="text-red-500 py-4">Failed Load Reviews</div>;
      ...
      <div className="relative z-10 text-white">
        ...
        <div className="flex justify-center items-center min-h-[200px]">
          <div className="bg-black border-2 border-purple-900 backdrop-blur-sm rounded-xl p-6 text-center">
            <p className="text-gray-300 text-lg">Nobody Reviews This Manga</p>
          </div>
        </div>
      </div>
    );
