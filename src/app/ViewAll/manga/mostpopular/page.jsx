...
const Page = () => {
  ...
  const fetchData = async (currentPage) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/top/manga?filter=bypopularity&page=${currentPage}`
      );
  ...
  return (
    <>
      <HeaderMenu title="Most Popular Manga" />

      <Pagination page={page} lastPage={lastPage} setPage={setPage} />

      {!loading && mostpopular?.data ? (
        <ListPaginationManga api={mostpopular} />
      ) : (
