...
const Page = () => {
  ...
  const fetchData = async (currentPage) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/seasons/upcoming?page=${currentPage}`
      );
  ...
  const lastPage = upcominganime?.pagination?.last_visible_page || 1;

  return (
    <>
      <HeaderMenu title="Top Upcoming Anime" />

      <Pagination page={page} lastPage={lastPage} setPage={setPage} />

      {!loading && upcominganime?.data ? (
        <ListPaginationAnime api={upcominganime} />
      ) : (
