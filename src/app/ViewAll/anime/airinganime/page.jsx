...
const Page = () => {
  ...
  const fetchData = async (currentPage) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/top/anime?filter=airing&page=${currentPage}`
      );
  ...
  const lastPage = topairinganime?.pagination?.last_visible_page || 1;
  ...
  return (
    <>
      ...
      {!loading && topairinganime?.data ? (
        <ListPaginationAnime api={topairinganime} />
      ) : (
