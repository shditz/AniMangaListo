...
const Page = () => {
  ...
  const fetchData = async (currentPage) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/top/anime?filter=favorite&page=${currentPage}`
      );
  ...
  return (
    <>
      <HeaderMenu title="Most Favorited Anime" />

      <Pagination page={page} lastPage={lastPage} setPage={setPage} />

      {!loading && mostfavorite?.data ? (
        <ListPaginationAnime api={mostfavorite} />
      ) : (
